...
include("OP_metal_opacity_comparison_funcs.jl")
...
function assert_allclose(actual, reference; rtol = 1e-7, atol = 0.0, err_msg = nothing,
    ...
    push!(lines,
          @sprintf("The arrays aren't consistent to within atol = %e, rtol = %e", atol, rtol))
...
@testset "TOPbase bound-free opacities" begin
    ...
    @testset "$species_name comparison" for species_name in ["H_I", "He_II"]
        λ_vals = _OP_hydrogenic_dflt_λ_vals(species_name)

        # compute the absorption coefficients using the TOPbase data
        hydrogenic_α_OP = calc_hydrogenic_bf_absorption_coef(λ_vals, T, ndens_species,
                                                             species_name; use_OP_data = true)
        # compute the absorption coefficients using our function for hydrogenic atoms
        hydrogenic_α_dflt = calc_hydrogenic_bf_absorption_coef(λ_vals, T, ndens_species,
                                                               species_name; use_OP_data = false)

        λ_comp_intervals = _OP_hydrogenic_λ_comp_intervals(species_name)
        comp_ind = _contained_in_inclusive_intervals(λ_vals, λ_comp_intervals)
        assert_allclose_grid(hydrogenic_α_OP[comp_ind], hydrogenic_α_dflt[comp_ind],
                             [("λ", λ_vals[comp_ind], "Å"),];
                             rtol = _OP_hydrogenic_rtol(species_name), atol = 0.0,
                             err_msg = string(species_name, "bf absorption coefficients computed ",
                                              "using data from the opacity\nproject are ",
